...
def define_R(opt):
    gpu_ids = opt['gpu_ids']
    __opt = opt['train']['recloss']
    if __opt['name'] == 'sphere':
        import models.modules.arch_sphere as arch
        netR = arch.SphereFeatureExtractor(feature=True)
        netR.load_state_dict(__opt['path'])
        if gpu_ids:
            netR.cuda()
            netR = nn.DataParallel(netR)
        netR.eval()  # No need to train
    elif __opt['name'] == 'lightcnn':
        import models.modules.arch_lightcnn as arch
        netR = arch.LightCNN_29Layers_v2()
        netR.eval()
        if gpu_ids:
            netR = torch.nn.DataParallel(netR).cuda()
        checkpoint = torch.load(__opt['path'])
