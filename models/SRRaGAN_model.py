...
class SRRaGANModel(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            if self.cri_fea:  # feature loss
                ...
                l_g_total += l_g_fea
            if self.cri_rec: # recognition loss
                real_fea = self.netR(self.var_H).detach()
                fake_fea = self.netR(self.fake_H)
                l_g_rec = self.l_rec_weight * self.cri_rec(fake_fea, real_fea, torch.ones(1).to(self.device))
                l_g_total += l_g_rec
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            if self.cri_rec:
                self.log_dict['l_g_rec'] = l_g_rec.item()
    ...
    def get_current_visuals(self, need_HR=True, isTrain=False):
        out_dict = OrderedDict()
        out_dict['LR'] = self.var_L.detach()[0].float().cpu()
        out_dict['SR'] = self.fake_H.detach()[0].float().cpu()
        if need_HR:
            out_dict['HR'] = self.var_H.detach()[0].float().cpu()
        if isTrain and self.opt['train']['recloss'] != '':
            out_dict['SRgray'] = self.makegray(self.fake_H)[0].detach().float().cpu()
            out_dict['HRgray'] = self.makegray(self.var_H)[0].detach().float().cpu()
        return out_dict
